...
public class WorkflowServiceImpl implements IWorkflowService {
	...
	public List<Task> findTaskListByName(String name) {
		List<Task> list = taskService.createTaskQuery()//
				...
				.list();
		/* 根据用户姓名查询当前任务可执行人 */
		List<Task> listcandidate = taskService.createTaskQuery()//
				.taskCandidateUser(name)// 指定组任务查询
				.orderByTaskCreateTime().asc()//
				.list();
		/* 将组任务查询出的结果一起并入个人任务的集合中，这样使得查询的结果全 */
		list.addAll(listcandidate);
	...
	public void saveSubmitTask(WorkflowBean workflowBean) {
		...
		//如果获取的连线名称是空，则就是从分派过来的
		if(outcome==null){
			outcome="分派";
		}
		System.out.println("连线名称是："+outcome);
		// 批注信息
		String message = workflowBean.getComment();
		// 获取请假单ID
		Long id = workflowBean.getId();
		System.out.println("请假单信息是："+id);
		/**
		...
		/**
		 * 做一个判断如果当前任务名称为处理人审批，则需要将下一步的审批人员设置为上一步分派的人员
		 * */
		if(task.getName().equals("处理人审批")){
			List<String> approverlist = new ArrayList<>();
			//获取批注信息，传递的是当前任务ID，获取历史任务ID对应的批注
			List<Comment> commentlist = findCommentByTaskId(task.getId());;
			if(commentlist!=null&&!commentlist.equals("")){
				//获取上一步审批的人员名称，则是需要设置的下一步的审批人员
				System.out.println("上一步的审批人员是"+commentlist.get(0).getUserId());
				approverlist.add(commentlist.get(0).getUserId());
				SessionContext.get().setAPProverlist(approverlist);//设置审批的领导集合
			}
		}
		/**
	...
	/**
	 * 根据任务ID，作废这个任务，也就是将获取的流程实例进行删除，并改变相对应的请假单状态
	 */
	@Override
	public void deleteLeaveBillById(Long id) {
		// TODO Auto-generated method stub
		String taskId = id.toString();// 任务id
		// 使用任务ID，查询任务对象
		Task task = taskService.createTaskQuery()//
				.taskId(taskId)// 使用任务ID查询
				.singleResult();
		System.out.println("任务id" + taskId);
		LeaveBill leaveBill = findLeaveBillByTaskId(taskId);// 通过task找到请假单id
		System.out.println(leaveBill.getId() + "请假单ID");
		leaveBill.setState(3);// 将此作废的请假单改变状态
		// 获取流程实例ID
		String processInstanceId = task.getProcessInstanceId();
		// runtimeService.suspendProcessInstanceById(processInstanceId);//挂起流程
		runtimeService.deleteProcessInstance(processInstanceId, "申请被驳回，申请人主动作废此任务");// 删除流程

	}

	/*
	 * 根据任务ID，将组任务的办理人查询出来然后全部删除，之后再添加进去更新的人员
	 */
	@Override
	public void updateCandidateUser(String taskId, List<String> approverlist) {
		// TODO Auto-generated method stub	
		//获取当前task的审批人的id，进行删除
		List<IdentityLink> identityLinks = taskService.getIdentityLinksForTask(taskId); 
		for (int i = 0; i < identityLinks.size(); i++) {
			String userid =identityLinks.get(i).getUserId();
			System.out.println("此任务的审批人有"+userid);
			taskService.deleteCandidateUser(taskId, userid);
		}
		//获取传递过来的审批人的id，对task审批人进行更新
		for (int j = 0; j < approverlist.size(); j++) {
			String approver = approverlist.get(j);
			System.out.println("此任务的更新后的办理人有"+approver);
			taskService.addCandidateUser(taskId, approver);
		}
		taskService.deleteCandidateUser(taskId, approverlist.get(0).trim());//删除第一个添加的部门名称，因为他不是审批人
	}
